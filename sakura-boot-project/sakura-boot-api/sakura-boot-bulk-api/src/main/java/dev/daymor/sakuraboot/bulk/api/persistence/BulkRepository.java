/*
 * Copyright (C) 2023-2025 Malcolm Rozé.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      https://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package dev.daymor.sakuraboot.bulk.api.persistence;

import java.io.Serializable;
import java.util.List;

import dev.daymor.sakuraboot.DataPresentation;

/**
 * The base interface for all Spring Data repositories with bulk operations.
 * The persistence context will be cleared after the flush by default.
 * <p>
 * <b>Example:</b>
 * </p>
 * <p>
 * To create a repository for your {@link DataPresentation} that inherits from
 * {@link BulkRepository}, follow these steps:
 * </p>
 * <blockquote>
 *
 * <pre>
 * public interface YourEntityRepository
 *     extends BulkRepository&lt;YourEntity, YourIdType&gt; {}
 * </pre>
 *
 * </blockquote>
 *
 * @param  <E> The entity type extending {@link DataPresentation}.
 * @param  <I> The ID of type Comparable and Serializable.
 * @author     Malcolm Rozé
 * @since      0.1.2
 */
public interface BulkRepository<E extends DataPresentation<I>,
    I extends Comparable<? super I> & Serializable> {

    /**
     * Inserts a collection of entities in the underlying data storage.
     *
     * @param  entities The collection of entities to insert.
     * @return          The inserted entities.
     */
    List<E> bulkInsert(Iterable<E> entities);

    /**
     * Updates a collection of entities in the underlying data storage.
     *
     * @param  entities The collection of entities to update.
     * @return          The updated entities
     */
    List<E> bulkUpdate(Iterable<E> entities);

    /**
     * Indicates whether the persistence context should be cleared after the
     * flush.
     * If the persistence context is cleared, it is better for memory usage
     * with large data sets.
     * But it will remove the ability to retrieve the proxy entities in
     * future operations.
     * {@code true} by default.
     * <p>
     * <b>NOTE:</b> The return value should be turned to {@code false} if the
     * application is using mapper with relationship objects to map to the dto.
     * </p>
     *
     * @return {@code true} if the persistence context should be cleared after
     *         the flush, {@code false} otherwise.
     */
    default boolean clearContextAfterFlush() {

        return true;
    }
}
