/*
 * Copyright (C) 2025 Malcolm Rozé.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      https://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package dev.daymor.sakuraboot.test.basic.api.presentation.controllers;

import java.io.Serializable;

import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;
import org.mockito.InjectMocks;
import org.mockito.Mock;
import org.springframework.http.HttpStatus;
import org.springframework.http.ResponseEntity;

import dev.daymor.sakuraboot.DataPresentation;
import dev.daymor.sakuraboot.basic.api.business.services.DeleteByIdService;
import dev.daymor.sakuraboot.basic.api.presentation.controllers.DeleteByIdController;
import dev.daymor.sakuraboot.test.SuperControllerTest;

import static org.assertj.core.api.Assertions.assertThat;
import static org.mockito.ArgumentMatchers.any;
import static org.mockito.Mockito.times;
import static org.mockito.Mockito.verify;

/**
 * The base test interface for all delete by id controllers. This interface
 * provides common tests for testing {@link DeleteByIdController}.
 *
 * <p>
 * <b>Example:</b>
 * </p>
 *
 * <p>
 * To create a concrete controller test class that inherits from
 * {@link DeleteByIdControllerTest}, follow these steps:
 * </p>
 *
 * <p>
 * Implements the {@link DeleteByIdControllerTest} class:
 * </p>
 * <blockquote>
 *
 * <pre>
 * public class YourServiceTest
 *     implements DeleteByIdControllerTest&lt;YourEntity, YourIdType&gt; {
 *
 *     private YourUtil util = new YourUtil();
 *
 *     &#064;InjectMocks
 *     private YourController controller;
 *
 *     &#064;Mock
 *     private YourService service;
 *
 *     &#064;Override
 *     public YourUtil getUtil() {
 *
 *         return util;
 *     }
 *
 *     &#064;Override
 *     public YourController getController() {
 *
 *         return controller;
 *     }
 *
 *     &#064;Override
 *     public YourService getService() {
 *
 *         return service;
 *     }
 * }
 * </pre>
 *
 * </blockquote>
 *
 * @param  <E> The {@link DataPresentation} type use in the service layer.
 * @param  <I> The ID of type Comparable and Serializable.
 * @author     Malcolm Rozé
 * @see        DeleteByIdController
 * @see        SuperControllerTest
 * @since      0.1.0
 */
public interface DeleteByIdControllerTest<E extends DataPresentation<I>,
    I extends Comparable<? super I> & Serializable>
    extends SuperControllerTest<E, I> {

    /**
     * Get the {@link DeleteByIdController} to test. Need to be
     * {@link InjectMocks}.
     *
     * @return A {@link DeleteByIdController}.
     */
    @Override
    DeleteByIdController<E, I> getController();

    /**
     * Get the {@link DeleteByIdService} for test. Need to be {@link Mock}.
     *
     * @return A {@link DeleteByIdService}.
     */
    @Override
    DeleteByIdService<E, I> getService();

    @Test
    @DisplayName("GIVEN a valid ID,"
        + " WHEN deleting by ID,"
        + " THEN the controller should return no content")
    default void testDeleteById() {

        // GIVEN
        final I validId = getValidId();

        // WHEN
        final ResponseEntity<Void> response
            = getController().deleteById(validId);

        // THEN
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NO_CONTENT);
        assertThat(response.getBody()).isNull();
        verify(getService(), times(1)).deleteById(any());
    }
}
